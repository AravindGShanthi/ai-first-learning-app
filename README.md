...
**Why**
Goal of this AI powered application is to generate dynamic custom course content based on user preference, so that the user can generate, learn and practice daily.

**Future Scope**
Application can be extended into an Mobile or Web application to generate dynamic custom course and later consumed as bite-size learnings.  Once the generated topic and lesson plan is accepted by the user, and the content can be generated by LLM backend service by nightly jobs, so that it will be ready by the morning for the user to consume.  
